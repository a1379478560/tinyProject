...
def apply_capture(board, capture_path):
    ...
    for i in range(len(capture_path)-1):
        ...
        if board.get(*indexify(capture_path[i+1])).is_black() and capture_path[i+1][0]=="h":
            board.get(*indexify(capture_path[i+1])).turn_king()
        if board.get(*indexify(capture_path[i+1])).is_white() and capture_path[i+1][0]=="a":
            board.get(*indexify(capture_path[i+1])).turn_king()
