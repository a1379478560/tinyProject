...
def getallid(s):
    ...
    for i in range(1,4):
        ...
        soup=BeautifulSoup(r.text,'html.parser')
        all_id=all_id+list(map(f,soup.find_all('li',{'class': 'grey'})))
        all_id_r=[]
        for iii in  all_id:
            if iii!=0:
                all_id_r.append(iii)
...
def getxls(id,s,ii):
    ...
    for i in range(1,n+1):
        ...
        for x in tr2:
            td=x.find_all('td')
            if len(td)==18 :
                sheet2.write(row2,0,td[0].text)
                sheet2.write(row2,1,float(td[1].text))
                sheet2.write(row2,2,toint(td[2].text))
                sheet2.write(row2,3,toint(td[3].text))
                sheet2.write(row2, 4, td[4].text)
                row2=row2+1
    wbk.save(r'spdex_com_detail/'+str(ii)+'.xls')
...
if __name__=='__main__':
    ...
    if not os.path.exists('spdex_com_detail'):
        os.mkdir('spdex_com_detail')
        print('创建spdex_文件夹')
    ii = 1  # 后面做文件名
